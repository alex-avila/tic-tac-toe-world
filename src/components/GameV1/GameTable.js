...
class GameTable extends Component {
    ...
    for (let i = 0; i < 3; i++) {
      for (let j = 0; j < 3; j++) {
        ...
        const box = (
          <li
            key={idName}
            className={prefixes[i] + ' ' + suffixes[j]}
            id={idName}
            onClick={this.playerDraw}
          />
        )
  ...
  playerDraw = e => {
    ...
    if (
      this.availableBoxes.indexOf(id) !== -1 &&
      !this.win &&
      this.turn === 'player'
    ) {
  ...
  computerDraw = () => {
    if (this.availableBoxes.length && this.turn === 'computer') {
      const box = this.availableBoxes[
        this.random(this.availableBoxes.length - 1)
      ]
  ...
  render() {
    return <ul className="grid">{this.generateBoxes()}</ul>
  }
}
